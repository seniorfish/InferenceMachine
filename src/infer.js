...
class Solver{
    ...
    constructor(conversions){
        conversions = conversions || makeConversionList()
        let book = new Map
        let trans = new Map
        for(let reaction of conversions){
            ...
            // update 3.24 针对不同条件有不同反应 solve里面同时更新
     ...
     */
    createNode(letter){
        if(!this.board.has(letter)){
            this.board.set(letter, {
                chu: new Set,
                ru: new Set,
                huhuan: new Set,
                fanying: new Set,
                possible: new Set(this.book.keys()),
                ext: new Array,
                trans: new Set      //  其实 Array 就够
            })
        }
    }
    /**
    ...
    */
    boardChu(a, b){
        this.createNode(a)
        this.createNode(b)
        this.board.get(a).chu.add(b)
        this.board.get(b).ru.add(a)
        if(this.board.get(a).ru.has(b)){     // 同时维护huhuan
            this.board.get(a).huhuan.add(b)
            this.board.get(b).huhuan.add(a)
        }
    }
    /**
     * 建立A与B反应的联系
    ...
    boardFanying(a, b){
        this.createNode(a)
        this.createNode(b)
        this.board.get(a).fanying.add(b)
        this.board.get(b).fanying.add(a)
    }
    /**
     * 语义化
     */
     ...
     */
    limit_containElements(letter, elements){
        let possible = this.board.get(letter).possible
        for(let guess of possible){
            let hasSameElem = false
            for(let elem of this.book.get(guess).elements){
     ...
     * 限制：确切的元素组成
     */
    limit_elements(letter, elements){
        let possible = this.board.get(letter).possible
        for(let guess of possible)
            if(!isSameSet(this.book.get(guess).elements, elements))
                possible.delete(guess)
    }
    /**
     * 限制：不同元素数量
     */
    limit_elementNumber(letter, num){
        let possible = this.board.get(letter).possible
        for(let guess of possible){
            if(this.book.get(guess).elements.size !== num)
                possible.delete(guess)
        }
    }
    /**
     ...
     * 可以替换上面的函数
     */
    limit_elementNumRange(letter, min, max){
        let possible = this.board.get(letter).possible
        for(let guess of possible){
            if(!(this.book.get(guess).elements.size >= min && this.book.get(guess).elements.size <= max))
                possible.delete(guess)
        }
    }
    /**
     ...
     */
    limit_oxide(letter){
        this.limit_elementNumber(letter, 2)
        this.limit_containElements(letter, new Set('O'))
    }
    /**
     * 限制：直接限制可能答案
     */
    limit_possible(letter, possible){
        keep(this.board.get(letter).possible, possible)
    }
    limit_transform(left, right){
        left.forEach(v1 => left.forEach(v2 => v1!==v2?this.boardFanying(v1, v2):null))
        left.forEach(l => right.forEach(r => this.boardChu(l, r)))
        for(let letter of [...left, ...right]){     // 排序除重，非必要
            this.board.get(letter).trans.add({
                left: `${[...left].sort().join('')}`,
    ...
    boardLimit(){
        ...
        for(let letter of this.board.keys()){
    ...
    input(s){
        ...
        if(s.match(/^[A-Z]([<>\=\-][A-Z])+$/i)){
            for(let i=1;i<s.length;i+=2){
                ...
                if(s[i] == '=')
                    this.boardHuhuan(a, b)
                else if(s[i] == '>')
                    this.boardChu(a, b)
                else if(s[i] == '<')
                    this.boardChu(b, a)
                else if(s[i] == '-')
                    this.boardFanying(a, b)
                else
                    throw new Error()
            }
            return 0
        ...
        if(s.match(/^[A-Z](\+[A-Z])*=[A-Z](\+[A-Z])*/i)){
            ...
            this.limit_transform(left, right)
            return 0
        }
        // 指定物质
        if(result = s.match(/^[a-zA-Z]:(?:([A-Z][\w()]*)|\[([A-Z][\w()]*(?:,[A-Z][\w()]*)*)\])$/)){
            if(result[1])
                this.limit_possible(s[0].toUpperCase(), new Set([result[1]]))
            else
                this.limit_possible(s[0].toUpperCase(), new Set(result[2].split(',')))
            return 0
        }
        if(result = s.match(/^\[([a-zA-Z]+)\]/)){
            ...
            if(lim = part.match(/^\[((?:[A-Z]\w*)+)\]$/)){
                letters.forEach(v => this.limit_containElements(v.toUpperCase(), formulaToElements(lim[1])))
            }else if(lim = part.match(/^\{((?:[A-Z]\w*)+)\}$/)){
                letters.forEach(v => this.limit_madeOf(v.toUpperCase(), formulaToElements(lim[1])))
            }else if(lim = part.match(/^\(O\)$/)){
                letters.forEach(v => this.limit_oxide(v.toUpperCase()))
            }else if(lim = part.match(/^\(([0-9]+-[0-9]+)\)$/)){
                let [a, b] = lim[1].split('-')
                // [a, b] = [parseInt(a), parseInt(b)].sort()
                letters.forEach(v => this.limit_elementNumRange(v.toUpperCase(), parseInt(a), parseInt(b)))  // 排序细节防呆
            }else if(lim = part.match(/^\(([0-9])\)$/)){
                letters.forEach(v => this.limit_elementNumber(v.toUpperCase(), parseInt(lim[1])))
            }else{
                return 1
            }
            return 0
        }
        if(s.length === 1 && isletter(s)){
            this.createNode(s.toUpperCase())
            return 0
        }
        return 1;
    }
...
export {Solver, makeConversionList, formulaToElements}
